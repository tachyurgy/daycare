...
export default function PortalParent(): JSX.Element {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: () => portalApi.parentSession(token!),
    enabled: validToken,
  });

  const uploadMutation = useMutation({
    mutationFn: ({ file, requiredDocId }: { file: File; requiredDocId: string }) =>
      portalApi.parentUpload(token!, requiredDocId, file),
    onSuccess: () => {
      toast.success('Upload received.');
      void qc.invalidateQueries({ queryKey: ['parent-portal', token] });
    },
    onError: () => {
  ...
  if (!validToken) {
    return (
      <PortalShell>
        <ErrorState title="This link isn't valid" description="Please ask the provider for a new one." />
      </PortalShell>
    );
  }
    ...
    return (
      <PortalShell>
        <ErrorState
          ...
          description={
            error instanceof Error ? error.message : 'Ask the provider to send a new link.'
          }
        />
      </PortalShell>
    );
    ...
    <PortalShell>
      <div className="text-center">
        <h1 className="text-xl font-semibold text-gray-900">
          Welcome, {data.child.firstName}'s family
        </h1>
        <p className="mt-1 text-sm text-gray-600">
          {data.providerName} needs a few documents before {data.child.firstName}'s first day.
        </p>
      </div>
...
function PortalShell({ children }: { children: React.ReactNode }): JSX.Element {
  return (
