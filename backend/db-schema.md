...
This document is a narrative tour of the SQLite schema. For authoritative
definitions, always read the migration files in `backend/migrations/`.

## Conventions

- **SQLite 3.40+** via the pure-Go `modernc.org/sqlite` driver. One file per
  environment; WAL mode; pragmas set at `db.Open`. See ADR-017.
...
- **Every table has** `created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP`
  and `updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP`. Timestamps are
  ISO 8601 UTC strings. Per-table `AFTER UPDATE` triggers keep `updated_at`
  current when the caller doesn't set it explicitly (guarded against
  recursion via `WHEN NEW.updated_at IS OLD.updated_at`).
- **Soft-deletable tables** additionally have `deleted_at TEXT NULL`. All
  read queries filter `WHERE deleted_at IS NULL`.
- **Enums** are expressed as `TEXT` + `CHECK (col IN (...))` constraints.
- **Emails** use `TEXT COLLATE NOCASE` (replaces Postgres `CITEXT`).
- **JSON columns** are `TEXT` with `CHECK (json_valid(col))`. SQLite's
  built-in `json1` functions (`json_valid`, `json_type`, `json_extract`,
  `json_array_length`) replace the `jsonb_*` operators we previously used.
- **Binary columns** are `BLOB` (replaces Postgres `BYTEA`).
- **IP address columns** are `TEXT` (no native `INET` type).
- **Foreign keys** — SQLite only enforces them when `PRAGMA foreign_keys
  = ON`, which `db.Open` sets. Deferred FK creation via
  `ALTER TABLE ADD CONSTRAINT` is NOT supported; one column
  (`signatures.consent_version_id` → `policy_versions.id`) relies on
  application-layer integrity for that reason.
...
Raw tokens are **never** stored — only `hmac-sha256(token_raw)` as `BLOB`.
The raw token is delivered via email/SMS and never persisted after the
send callback. `subject_id` is polymorphic (no FK) and its meaning depends
on `kind`:
...
A child is attached to one provider, has a TEXT-JSON `guardians` array
...
guardians column is validated with `json_valid()` + `json_type(col) =
'array'`. The checklist is seeded by the application from a state/age
template and its `status` is the heartbeat of the compliance dashboard.
...
identifiers and system-cron actors fit. SQLite has no per-table GRANTs, so
mutation-protection relies on the `internal/audit` package only exporting
`Insert` — no `UPDATE` or `DELETE` helpers exist.
...
- **Row-level security.** SQLite doesn't have it; tenant isolation is
  enforced in the repository layer.
- **Native ENUM types.** SQLite doesn't have them; we use `CHECK`
  constraints instead.
- **Materialized views for the dashboard.** `compliance_snapshots` is our
  materialization — explicit, not magical.
- **Table partitioning on `audit_log`.** Not applicable under SQLite.
  Revisit (probably by rotating the DB file into a cold archive on a
  schedule) if the file grows past ~10 GB.
